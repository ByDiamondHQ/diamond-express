...
const login = async (req, res) => {
  ...
  if (!magicToken) {
    failure(res, { success: false, error: "Token does not exist" }, 404)
  }

  if (!magicToken.active) {
    failure(res, { success: false, error: "Token has expired" }, 404)
  }
  ...
  return success(res, { success: true, data: { message: "Login token sent", token: jwtToken } }, 200)
}
